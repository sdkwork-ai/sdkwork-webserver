...
# Remember where the caller invoked us from, before anything can change cwd.
# User-supplied relative paths (--out, -o, -i, --export) are anchored here so
# they resolve against the caller's directory, not the module root.
SDKWORK_BIN_INVOCATION_DIR="$PWD"
export SDKWORK_BIN_INVOCATION_DIR
