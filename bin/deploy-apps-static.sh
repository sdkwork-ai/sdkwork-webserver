#!/usr/bin/env bash
# deploy-apps-static.sh — build, package and publish the Adaptive Web static
# dist (PC / H5) of any independent sibling module to a target host (local
# WSL or remote ssh), with extraction, verification and quick rollback.
...
# bootstrap chain before loading it, so the script works from any directory
# in any shell context (Git Bash, WSL native, Linux).
sdkwork_deploy_apps_static_strip_crlf() {
  ...
  for f in "${bin_dir}/lib/apps-static.sh" \
           "${bin_dir}/lib/apps-static-deploy.sh" \
...
SDKWORK_ENTRY="deploy-apps-static"
