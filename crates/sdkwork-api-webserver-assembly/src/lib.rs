...
// `framework_observability`, for the standalone-gateway integration points
// (`seed_space_repository`, `web_module`, `web_module_with_context`), or for the
// owner-API repository re-exports below. This marker keeps the file authored;
// without it the next `pnpm api:assembly:materialize` silently drops every one
// of them and the crate stops compiling, because `bootstrap.rs` still does
