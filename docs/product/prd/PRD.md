...
| PC management application | `apps/sdkwork-webserver-pc` provides an isolated tenant Console and lazy backend-admin surface. Console application, domain, and certificate operations consume `@sdkwork/deployments-app-sdk`; application-package bytes are uploaded only through `@sdkwork/drive-app-sdk`; internal operator workflows consume only `@sdkwork/webserver-backend-sdk`. All authenticated SDK clients share the bootstrap-owned TokenManager. |
...
- `PRD-FR-030`: The tenant-bound Backend context never becomes cross-tenant because an account has `web.*` or `*`; cross-tenant platform administration requires a separate platform-admin route and authorization chain. Host-scoped surfaces (cluster plane, server files explorer, Web Server configuration catalog) therefore answer only the *platform operator tenant*, resolved through `web_platform_operator_tenant_id()` (`SDKWORK_WEBSERVER_PLATFORM_OPERATOR_TENANT_ID`, defaulting to the IAM bootstrap tenant `100001`). Tenant membership is necessary but not sufficient: the operation's `web.*` permission is still required, and a `*` grant alone never reaches these surfaces.
...
- `PRD-FR-036`: Root domains are explicit tenant-owned Zones. Every apex or subdomain hostname belongs to one Zone and application routing is represented by `webserver_site_binding`, never a direct application column on the hostname.
