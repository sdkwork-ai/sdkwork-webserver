...
final class DeploymentPage
{
    ...
    public function __construct(array $data = [])
    {
        $this->items = array_key_exists('items', $data)
            ? is_array($data['items'])
                ? array_values(array_map(static fn($item) => is_array($item) ? DeploymentResponse::fromArray($item) : $item, $data['items']))
                : []
            : [];
    ...
    {
        return [
            'items' => array_values(array_map(static fn($item) => $item instanceof DeploymentResponse ? $item->toArray() : $item, $this->items)),
            'total' => $this->total,
        ];
    }
}
