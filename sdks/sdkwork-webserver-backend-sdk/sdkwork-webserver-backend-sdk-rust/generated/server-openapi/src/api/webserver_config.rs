...
use crate::models::{WebserverConfigCatalog, WebserverConfigFile, WebserverConfigWriteRequest, WebserverConfigWriteResult};

#[derive(Clone)]
pub struct WebserverConfigApi {
    client: Arc<SdkworkHttpClient>,
}

impl WebserverConfigApi {
    pub fn new(client: Arc<SdkworkHttpClient>) -> Self {
        Self { client }
    }

    /// List the managed Web Server configuration catalog
    pub async fn webserver_configs_list(&self) -> Result<WebserverConfigCatalog, SdkworkError> {
        let path = backend_path(&"/webserver_configs".to_string());
        self.client.get(&path, None, None).await
    }

    /// Read one managed configuration file
    pub async fn webserver_configs_retrieve(&self, config_id: &str) -> Result<WebserverConfigFile, SdkworkError> {
        let path = backend_path(&format!("/webserver_configs/{}", serialize_path_parameter(config_id, PathParameterSpec::new("configId", "simple", false))));
        self.client.get(&path, None, None).await
    }

    /// Validate and atomically overwrite one managed configuration file
    pub async fn webserver_configs_update(&self, config_id: &str, body: &WebserverConfigWriteRequest, idempotency_key: &str) -> Result<WebserverConfigWriteResult, SdkworkError> {
        let path = backend_path(&format!("/webserver_configs/{}", serialize_path_parameter(config_id, PathParameterSpec::new("configId", "simple", false))));
