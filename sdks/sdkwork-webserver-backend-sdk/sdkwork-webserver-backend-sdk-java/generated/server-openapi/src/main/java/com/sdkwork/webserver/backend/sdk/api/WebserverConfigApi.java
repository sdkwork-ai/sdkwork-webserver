...
public class WebserverConfigApi {
    ...
    public WebserverConfigApi(HttpClient client) {
        this.client = client;
    }

    /** List the managed Web Server configuration catalog */
    public WebserverConfigsListResponse webserverConfigsList() throws Exception {
        Object raw = client.get(ApiPaths.backendPath("/webserver_configs"));
        return client.convertValue(raw, new TypeReference<WebserverConfigsListResponse>() {});
    }

    /** Read one managed configuration file */
    public WebserverConfigsRetrieveResponse webserverConfigsRetrieve(String configId) throws Exception {
        Object raw = client.get(ApiPaths.backendPath("/webserver_configs/" + serializePathParameter(configId, new PathParameterSpec("configId", "simple", false)) + ""));
        return client.convertValue(raw, new TypeReference<WebserverConfigsRetrieveResponse>() {});
    }

    /** Validate and atomically overwrite one managed configuration file */
    public WebserverConfigsUpdateResponse webserverConfigsUpdate(String configId, WebserverConfigWriteRequest body, String idempotencyKey) throws Exception {
        Map<String, String> requestHeaders = buildRequestHeaders(
                Map.of("Idempotency-Key", new HeaderParameterSpec(idempotencyKey, "simple", false, null)),
                Map.of()
        );
        Object raw = client.put(ApiPaths.backendPath("/webserver_configs/" + serializePathParameter(configId, new PathParameterSpec("configId", "simple", false)) + ""), body, null, requestHeaders, "application/json");
        return client.convertValue(raw, new TypeReference<WebserverConfigsUpdateResponse>() {});
    }
