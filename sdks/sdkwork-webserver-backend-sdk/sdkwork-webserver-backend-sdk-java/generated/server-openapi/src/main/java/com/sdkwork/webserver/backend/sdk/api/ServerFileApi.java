...
public class ServerFileApi {
    ...
    public ServerFileApi(HttpClient client) {
        this.client = client;
    }

    /** List Server Files deployment nodes */
    public ServerFilesNodesListResponse serverFilesNodesList() throws Exception {
        Object raw = client.get(ApiPaths.backendPath("/server_files/nodes"));
        return client.convertValue(raw, new TypeReference<ServerFilesNodesListResponse>() {});
    }
    ...
    public ServerFilesNodeOperationsListResponse serverFilesNodeOperationsList(String nodeId, String path) throws Exception {
        String query = buildQueryString(List.of(
            new QueryParameterSpec("path", path, "form", true, false, null)
        ));
        Object raw = client.get(ApiPaths.appendQueryString(ApiPaths.backendPath("/server_files/nodes/" + serializePathParameter(nodeId, new PathParameterSpec("nodeId", "simple", false)) + "/operations"), query));
        return client.convertValue(raw, new TypeReference<ServerFilesNodeOperationsListResponse>() {});
    }

    /** Run a project operation on a deployment node */
    public ServerFilesNodeOperationsCreateResponse201 serverFilesNodeOperationsCreate(String nodeId, ServerRunOperationRequest body) throws Exception {
        Object raw = client.post(ApiPaths.backendPath("/server_files/nodes/" + serializePathParameter(nodeId, new PathParameterSpec("nodeId", "simple", false)) + "/operations"), body, null, null, "application/json");
        return client.convertValue(raw, new TypeReference<ServerFilesNodeOperationsCreateResponse201>() {});
    }
