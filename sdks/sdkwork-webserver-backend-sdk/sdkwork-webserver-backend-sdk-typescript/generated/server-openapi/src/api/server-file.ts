...
import type { ServerDirectoryListing, ServerFileContent, ServerFilesNode, ServerOperationResult, ServerProjectOperations, ServerRunOperationRequest } from '../types';


export interface ServerFileNodeOperationsListParams {
  path: string;
}

export class ServerFileNodeOperationsApi {
...
/** List operations available for a project directory */
  async list(nodeId: string, params: ServerFileNodeOperationsListParams, requestOptions?: ApiRequestOptions): Promise<ServerProjectOperations> {
    const query = buildQueryString([
      { name: 'path', value: params.path, style: 'form', explode: true, allowReserved: false },
    ]);
    return this.client.request<ServerProjectOperations>(appendQueryString(backendApiPath(`/server_files/nodes/${serializePathParameter(nodeId, { name: 'nodeId', style: 'simple', explode: false })}/operations`), query), { ...(requestOptions?.signal !== undefined ? { signal: requestOptions.signal } : {}), ...(requestOptions?.timeout !== undefined ? { timeout: requestOptions.timeout } : {}), method: 'GET' as any, sdkworkUnwrapKind: 'data' });
  }

/** Run a project operation on a deployment node */
  async create(nodeId: string, body: ServerRunOperationRequest, requestOptions?: ApiRequestOptions): Promise<ServerOperationResult> {
    return this.client.request<ServerOperationResult>(backendApiPath(`/server_files/nodes/${serializePathParameter(nodeId, { name: 'nodeId', style: 'simple', explode: false })}/operations`), { ...(requestOptions?.signal !== undefined ? { signal: requestOptions.signal } : {}), ...(requestOptions?.timeout !== undefined ? { timeout: requestOptions.timeout } : {}), method: 'POST' as any, body, contentType: 'application/json', sdkworkUnwrapKind: 'data' });
  }
}
...
export class ServerFileApi {
  public readonly nodes: ServerFileNodesApi;

  constructor(client: HttpClient) {
    this.nodes = new ServerFileNodesApi(client);
  }

}

export function createServerFileApi(client: HttpClient): ServerFileApi {
  return new ServerFileApi(client);
}
