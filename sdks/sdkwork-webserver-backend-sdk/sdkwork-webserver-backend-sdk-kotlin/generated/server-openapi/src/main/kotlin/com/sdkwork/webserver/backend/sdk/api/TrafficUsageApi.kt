...
class TrafficUsageApi(private val client: HttpClient) {

    /** Retrieve aggregated traffic usage of the caller's own tenant */
    suspend fun trafficUsagesRetrieve(dateFrom: String? = null, dateTo: String? = null, dimension: String? = null, topApps: Int? = null): TrafficUsagesRetrieveResponse? {
        val query = buildQueryString(listOf(
            QueryParameterSpec("date_from", dateFrom, "form", true, false, null),
            QueryParameterSpec("date_to", dateTo, "form", true, false, null),
            QueryParameterSpec("dimension", dimension, "form", true, false, null),
            QueryParameterSpec("top_apps", topApps, "form", true, false, null)
        ))
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.backendPath("/traffic_usage"), query))
        return client.convertValue(raw, object : TypeReference<TrafficUsagesRetrieveResponse>() {})
    }

    /** Retrieve aggregated traffic usage of every tenant */
    suspend fun platformTrafficUsagesRetrieve(dateFrom: String? = null, dateTo: String? = null, dimension: String? = null, topApps: Int? = null): PlatformTrafficUsagesRetrieveResponse? {
        val query = buildQueryString(listOf(
            QueryParameterSpec("date_from", dateFrom, "form", true, false, null),
            QueryParameterSpec("date_to", dateTo, "form", true, false, null),
            QueryParameterSpec("dimension", dimension, "form", true, false, null),
        ...
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.backendPath("/platform_traffic_usage"), query))
        return client.convertValue(raw, object : TypeReference<PlatformTrafficUsagesRetrieveResponse>() {})
    }
