...
class MetricsSummaryApi(private val client: HttpClient) {

    /** Retrieve the dashboard metric summary of the caller's own tenant */
    suspend fun metricsSummariesRetrieve(dateFrom: String? = null, dateTo: String? = null): MetricsSummariesRetrieveResponse? {
        val query = buildQueryString(listOf(
            QueryParameterSpec("date_from", dateFrom, "form", true, false, null),
            QueryParameterSpec("date_to", dateTo, "form", true, false, null)
        ))
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.backendPath("/metrics_summaries"), query))
        return client.convertValue(raw, object : TypeReference<MetricsSummariesRetrieveResponse>() {})
    }

    /** Retrieve the dashboard metric summary of every tenant */
    suspend fun platformMetricsSummariesRetrieve(dateFrom: String? = null, dateTo: String? = null): PlatformMetricsSummariesRetrieveResponse? {
        val query = buildQueryString(listOf(
            QueryParameterSpec("date_from", dateFrom, "form", true, false, null),
            QueryParameterSpec("date_to", dateTo, "form", true, false, null)
        ))
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.backendPath("/platform_metrics_summaries"), query))
        return client.convertValue(raw, object : TypeReference<PlatformMetricsSummariesRetrieveResponse>() {})
    }
