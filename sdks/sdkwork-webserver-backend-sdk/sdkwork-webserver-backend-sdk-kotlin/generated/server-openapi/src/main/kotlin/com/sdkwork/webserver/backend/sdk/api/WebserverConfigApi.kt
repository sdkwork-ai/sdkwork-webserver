...
class WebserverConfigApi(private val client: HttpClient) {

    /** List the managed Web Server configuration catalog */
    suspend fun webserverConfigsList(): WebserverConfigsListResponse? {
        val raw = client.get(ApiPaths.backendPath("/webserver_configs"))
        return client.convertValue(raw, object : TypeReference<WebserverConfigsListResponse>() {})
    }

    /** Read one managed configuration file */
    suspend fun webserverConfigsRetrieve(configId: String): WebserverConfigsRetrieveResponse? {
        val raw = client.get(ApiPaths.backendPath("/webserver_configs/${serializePathParameter(configId, PathParameterSpec("configId", "simple", false))}"))
        return client.convertValue(raw, object : TypeReference<WebserverConfigsRetrieveResponse>() {})
    }

    /** Validate and atomically overwrite one managed configuration file */
    suspend fun webserverConfigsUpdate(configId: String, body: WebserverConfigWriteRequest, idempotencyKey: String): WebserverConfigsUpdateResponse? {
        ...
        val raw = client.put(ApiPaths.backendPath("/webserver_configs/${serializePathParameter(configId, PathParameterSpec("configId", "simple", false))}"), body, null, requestHeaders, "application/json")
        return client.convertValue(raw, object : TypeReference<WebserverConfigsUpdateResponse>() {})
    }
