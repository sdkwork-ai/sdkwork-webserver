...
public class ServerFileApi {
    ...
    public func serverFilesNodesList() async throws -> ServerFilesNodesListResponse? {
        return try await client.get(ApiPaths.backendPath("/server_files/nodes"), responseType: ServerFilesNodesListResponse.self)
    }
    ...
    public func serverFilesNodeOperationsList(nodeId: String, path: String) async throws -> ServerFilesNodeOperationsListResponse? {
        let query = buildQueryString([
            QueryParameterSpec(name: "path", value: path, style: "form", explode: true, allowReserved: false, contentType: nil)
        ])
        return try await client.get(ApiPaths.appendQueryString(ApiPaths.backendPath("/server_files/nodes/\(serializePathParameter(nodeId, PathParameterSpec(name: "nodeId", style: "simple", explode: false)))/operations"), query), responseType: ServerFilesNodeOperationsListResponse.self)
    }

    /// Run a project operation on a deployment node
    public func serverFilesNodeOperationsCreate(nodeId: String, body: ServerRunOperationRequest) async throws -> ServerFilesNodeOperationsCreateResponse201? {
        return try await client.post(ApiPaths.backendPath("/server_files/nodes/\(serializePathParameter(nodeId, PathParameterSpec(name: "nodeId", style: "simple", explode: false)))/operations"), body: body, params: nil, headers: nil, contentType: "application/json", responseType: ServerFilesNodeOperationsCreateResponse201.self)
    }
