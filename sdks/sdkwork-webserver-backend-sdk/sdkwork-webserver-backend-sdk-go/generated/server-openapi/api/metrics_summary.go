...
type MetricsSummaryApi struct {
    client *sdkhttp.Client
}

func NewMetricsSummaryApi(client *sdkhttp.Client) *MetricsSummaryApi {
    return &MetricsSummaryApi{client: client}
}

// Retrieve the dashboard metric summary of the caller's own tenant
func (a *MetricsSummaryApi) MetricsSummariesRetrieve(dateFrom *string, dateTo *string) (sdktypes.MetricsSummariesRetrieveResponse, error) {
    ...
    raw, err := a.client.Get(AppendQueryString(BackendApiPath("/metrics_summaries"), query), nil, nil)
    if err != nil {
        var zero sdktypes.MetricsSummariesRetrieveResponse
        return zero, err
    }
    return decodeResult[sdktypes.MetricsSummariesRetrieveResponse](raw)
}

// Retrieve the dashboard metric summary of every tenant
func (a *MetricsSummaryApi) PlatformMetricsSummariesRetrieve(dateFrom *string, dateTo *string) (sdktypes.PlatformMetricsSummariesRetrieveResponse, error) {
    query := BuildQueryString([]QueryParameterSpec{
        {Name: "date_from", Value: func() interface{} { if dateFrom == nil { return nil }; return *dateFrom }(), Style: "form", Explode: true, AllowReserved: false},
        {Name: "date_to", Value: func() interface{} { if dateTo == nil { return nil }; return *dateTo }(), Style: "form", Explode: true, AllowReserved: false},
    })
    raw, err := a.client.Get(AppendQueryString(BackendApiPath("/platform_metrics_summaries"), query), nil, nil)
    if err != nil {
        var zero sdktypes.PlatformMetricsSummariesRetrieveResponse
        return zero, err
    }
    return decodeResult[sdktypes.PlatformMetricsSummariesRetrieveResponse](raw)
}
