...
class ServerFileApi:
    """server_file server_files API client."""

    def __init__(self, client: HttpClient):
        self._client = client
        self.nodes = ServerFileNodesApi(client)


class ServerFileNodesApi:
    """server_file server_files.nodes API client."""
    ...
    def list(self) -> ServerFilesNodesListResponse:
...
class ServerFileNodeOperationsApi:
    ...
    def list(self, node_id: str, path: str) -> ServerFilesNodeOperationsListResponse:
        """List operations available for a project directory"""
        query = build_query_string([
            {'name': 'path', 'value': path, 'style': 'form', 'explode': True, 'allow_reserved': False},
        ])
        return self._client.get(_append_query_string(f"/backend/v3/api/server_files/nodes/{serialize_path_parameter(node_id, {'name': 'nodeId', 'style': 'simple', 'explode': False})}/operations", query))

    def create(self, node_id: str, body: ServerRunOperationRequest) -> ServerFilesNodeOperationsCreateResponse201:
        """Run a project operation on a deployment node"""
        return self._client.post(f"/backend/v3/api/server_files/nodes/{serialize_path_parameter(node_id, {'name': 'nodeId', 'style': 'simple', 'explode': False})}/operations", json=body)
