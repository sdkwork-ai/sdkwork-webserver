...
from ..models import MetricsSummariesRetrieveResponse, PlatformMetricsSummariesRetrieveResponse
...
class MetricsSummaryApi:
    """metrics_summary metrics_summaries API client."""

    def __init__(self, client: HttpClient):
        self._client = client
        self.platform_metrics_summaries = MetricsSummaryPlatformMetricsSummariesApi(client)


    def list(self, date_from: Optional[str] = None, date_to: Optional[str] = None) -> MetricsSummariesRetrieveResponse:
        ...
        return self._client.get(_append_query_string(f"/backend/v3/api/metrics_summaries", query))

class MetricsSummaryPlatformMetricsSummariesApi:
    """metrics_summary metrics_summaries.platform_metrics_summaries API client."""

    def __init__(self, client: HttpClient):
        self._client = client


    def retrieve(self, date_from: Optional[str] = None, date_to: Optional[str] = None) -> PlatformMetricsSummariesRetrieveResponse:
        """Retrieve the dashboard metric summary of every tenant"""
        query = build_query_string([
            {'name': 'date_from', 'value': date_from, 'style': 'form', 'explode': True, 'allow_reserved': False},
            {'name': 'date_to', 'value': date_to, 'style': 'form', 'explode': True, 'allow_reserved': False},
        ])
        return self._client.get(_append_query_string(f"/backend/v3/api/platform_metrics_summaries", query))
