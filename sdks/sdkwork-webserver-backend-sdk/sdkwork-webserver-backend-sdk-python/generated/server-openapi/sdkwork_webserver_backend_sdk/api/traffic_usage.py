...
from ..models import PlatformTrafficUsagesRetrieveResponse, TrafficUsagesRetrieveResponse
...
class TrafficUsageApi:
    """traffic_usage traffic_usages API client."""

    def __init__(self, client: HttpClient):
        self._client = client
        self.platform_traffic_usages = TrafficUsagePlatformTrafficUsagesApi(client)


    def list(self, date_from: Optional[str] = None, date_to: Optional[str] = None, dimension: Optional[str] = None, top_apps: Optional[int] = None) -> TrafficUsagesRetrieveResponse:
        """Retrieve aggregated traffic usage of the caller's own tenant"""
        query = build_query_string([
            {'name': 'date_from', 'value': date_from, 'style': 'form', 'explode': True, 'allow_reserved': False},
            {'name': 'date_to', 'value': date_to, 'style': 'form', 'explode': True, 'allow_reserved': False},
            {'name': 'dimension', 'value': dimension, 'style': 'form', 'explode': True, 'allow_reserved': False},
            {'name': 'top_apps', 'value': top_apps, 'style': 'form', 'explode': True, 'allow_reserved': False},
        ])
        return self._client.get(_append_query_string(f"/backend/v3/api/traffic_usage", query))

class TrafficUsagePlatformTrafficUsagesApi:
    """traffic_usage traffic_usages.platform_traffic_usages API client."""

    def __init__(self, client: HttpClient):
        self._client = client


    def retrieve(self, date_from: Optional[str] = None, date_to: Optional[str] = None, dimension: Optional[str] = None, top_apps: Optional[int] = None) -> PlatformTrafficUsagesRetrieveResponse:
        ...
        return self._client.get(_append_query_string(f"/backend/v3/api/platform_traffic_usage", query))
