...
old2 = """    let names = expanded.iter().map(|d| d.name.as_str()).collect::<Vec<_>>();
    assert_eq!(names, vec!["server", "server", "return"]);"""
new2 = """    let names = expanded.iter().map(|d| d.name.as_str()).collect::<Vec<_>>();
...
# 5. add the http-level fail table + harness
old8 = """#[test]
...
new8 = """/// Fail-closed forms that live at the http level (upstream blocks).
