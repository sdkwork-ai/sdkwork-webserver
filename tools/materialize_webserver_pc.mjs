...
const packages = [
  ...
  // machine (VM) the authenticated operator provisioned, and the console
  // is where they apply for one, resize it, suspend it, or retire it. The
  // resource is served by that module's own app-api face
  // (`/app/v3/api/sandbox/sandbox_instances`, dual token, tenant and owner taken
  // from the verified principal), so this package is a host adapter rather than a
  // second implementation: it contributes the menu entry, the resource key, and
  // the page, and reads nothing the server did not already scope to the caller.
  //
  // It is mounted on the console only. The tenant-level inventory an operator
  // would need is a different plane with a different authorization argument, and
  // claiming the admin surface here would present this per-user page as that
  // inventory.
  //
  // The transport is composed by console-core (`createSandboxAppClient`), which
  // is the single place in the console allowed to own one; the module has no
  // generated SDK to compose against (sdkwork-sandbox keeps its SDK family
  // inactive for Phase 0), so core carries a hand-written client documented
  // against the route crate's payloads. See `sandbox-client.ts` in console-core.
  { id: "console-sandbox", surface: "app-console", capability: "sandbox", deps: { "@sdkwork/sdk-common": "workspace:*", "@sdkwork/ui-pc-react": "workspace:*", "@sdkwork/webserver-pc-commons": "workspace:*", "@sdkwork/webserver-pc-console-core": "workspace:*", react: "catalog:" }, module: [["sandbox-instances", "VM Instances", "Virtual machine (VM) instances provisioned by the authenticated user", "web.sandbox.read"]], extraIndexExports: ['export * from "./SandboxInstancesConsoleSurface.tsx";', 'export * from "./sandbox-instances-model.ts";', 'export * from "./i18n.ts";', 'export * from "./locale.tsx";'], dependencyPolicy: "Host adapter only. The page, the form vocabulary, and the i18n are authored here because the virtual machine instance registry has no shared upstream implementation; the transport and its wire types are consumed from @sdkwork/webserver-pc-console-core public exports, and raw HTTP in this package is forbidden.", sdkPolicy: "This package owns no transport and imports no generated SDK. It constructs the Sandbox app-api client through `createSandboxAppClient` re-exported by @sdkwork/webserver-pc-console-core and passes it the injected app-api base URL and IAM token manager, so the dual-token session and the transport stay owned by core.", readme: "This package owns the VM Instances capability on the app-console surface: the list of virtual machine (VM) instances the authenticated operator provisioned, plus provisioning, resizing, suspend/resume, and retirement. It is a host adapter over the sdkwork-sandbox app-api face — the page contributes the menu entry, the resource key, and the form vocabulary, while the dual-token transport is composed by `@sdkwork/webserver-pc-console-core`. Ownership is not a client-side filter: the server derives the tenant and the owner from the verified principal and answers only that slice, so the page cannot be pointed at another user's instances. The state machine is the server's as well — the page offers the transitions a state admits, and a transition the server refuses (409) is reported verbatim rather than optimistically applied." },
