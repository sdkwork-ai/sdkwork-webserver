...
module_nginx_sidecar_abs_path() {
  local module_root="$1"
  local environment="${2:-${SDKWORK_WEBSERVER_ENVIRONMENT:-development}}"
...
materialize_module_import_files() {
  local imports_root="${CONFIG_ROOT}/imports.d"
  local modules_root="${CONFIG_ROOT}/modules"
  local environment active_profile import_profile import_environments
...
# Commissioned environments: ${import_environments}
# Instance default environment: ${environment}  Import profile: ${import_profile}
EOF
...
EOF
    ...
    for module in "${module_list[@]}"; do
      ...
      if module_nginx_import_enabled "${module_root}"; then
        ...
        # Universal import plane (ENVIRONMENT_SPEC §6.2.2): include every
        # commissioned environment whose sidecar file actually exists, so one
        # webserver instance can route all lifecycle domain edges.
        for import_environment in ${import_environments}; do
          ...
          if [ -f "${sidecar_path}" ]; then
            ...
            log "module ${module} has no ${import_profile}.${import_environment} sidecar; skipped"
          fi
        done
        continue
      fi
